...
class CascadePolicy(nn.Module):
    """
    Cascade-integrated policy for METRA, managing an ensemble of base policies.
    Each base policy (for k>=2) outputs action and fallback lambda.
    The ensemble action is a convex combination.
    """
    def __init__(self, base_policy_cls, num_stages, policy_args, device, action_dim):
        super().__init__()
        self.base_policies = nn.ModuleList()
        self.num_stages = num_stages
        self.current_stage = 0
        self.device = device
        self.action_dim = action_dim
        # Initialize first base policy (no lambda)
        self._add_base_policy(base_policy_cls, policy_args, with_lambda=False)

    def _add_base_policy(self, base_policy_cls, policy_args, with_lambda):
        base_policy = base_policy_cls(**policy_args).to(self.device)
        if with_lambda:
            # Extend output to include lambda (scalar [0,1])
            original_module = base_policy.module
            extended_output_dim = self.action_dim + 1  # action + lambda
            extended_module = GaussianMLPTwoHeadedModuleEx(
                input_dim=original_module.input_dim,
                output_dim=extended_output_dim,
                hidden_sizes=original_module.hidden_sizes,
                hidden_nonlinearity=original_module.hidden_nonlinearity,
                layer_normalization=original_module.layer_normalization,
                max_std=original_module.max_std,
                normal_distribution_cls=original_module.normal_distribution_cls,
                output_w_init=original_module.output_w_init,
                init_std=original_module.init_std,
            )
            base_policy.module = extended_output_dim if policy_args.get('use_encoder') else extended_module
        self.base_policies.append(base_policy)
        self.current_stage += 1

    def forward(self, obs, skill, deterministic=False):
        # Recursive convex combination for ensemble action
        if self.current_stage == 1:
            return self.base_policies[0](obs, skill, deterministic=deterministic)
        else:
            # Get action from previous ensemble (recursive)
            prev_action = self.base_policies[:-1].forward(obs, skill, deterministic=deterministic)  # Assuming recursive call, but need to adjust
            # Last base policy outputs action and lambda
            base_out = self.base_policies[-1](obs, skill, deterministic=deterministic)
            base_action = base_out[:, :-1]  # action part
            lambda_k = torch.sigmoid(base_out[:, -1])  # lambda in [0,1]
            # Convex combination
            ensemble_action = lambda_k.unsqueeze(1) * prev_action + (1 - lambda_k.unsqueeze(1)) * base_action
            return ensemble_action

    def get_action(self, obs, skill, deterministic=False):
        with torch.no_grad():
            action = self.forward(obs, skill, deterministic=deterministic)
        return action, {}  # Adjust as per PolicyEx interface

    def parameters(self):
        # All base policies are trainable
        for policy in self.base_policies:
            for p in policy.parameters():
                yield p

    # Other methods to match PolicyEx interface (e.g., process_observations, etc.)

class DRQ_METRAAgent:
    """
    DrQ agent with METRA skill discovery integration, now with Cascade integration.
    ...
    def __init__(self,
            ...
            sac_lr_a=None,
            seed=0,
            num_cascade_stages=3,  # New param: number of cascade stages (K)
            epochs_per_stage=10,   # New param: epochs per stage before adding new policy
    ):
        self._env = env
        ...
        # Cascade-integrated skill_policy
        ...
        self.skill_policy = CascadePolicy(
            base_policy_cls=PolicyEx,
        ...
        self.param_modules = {
            ...
            'skill_policy': self.skill_policy,  # Now CascadePolicy
    ...
    def _get_trajectories(self,
        ...
        with MeasureAndAccTime(time_get_trajectories):
            trajectories = self.obtain_exact_trajectories( 
                env=self._env,
                policy=self.skill_policy,  # Now uses ensemble policy
    ...
    def train(self, n_epochs):
        ...
        with utils.GlobalContext({'phase': 'train', 'policy': 'sampling'}):
            ...
            for epoch in range(n_epochs):
                ...
                # Check if we need to add a new cascade stage
                self.current_epoch_in_stage += 1
                if self.skill_policy.current_stage < self.num_cascade_stages and self.current_epoch_in_stage >= self.epochs_per_stage:
                    self._add_cascade_stage()
                    self.current_epoch_in_stage = 0
    ...
    def _add_cascade_stage(self):
        """Add a new base policy to the cascade ensemble."""
        print(f"Adding cascade stage {self.skill_policy.current_stage + 1}")
        policy_args = {  # Reuse from init
            'name': f'skill_policy_stage{self.skill_policy.current_stage + 1}',
            'env_spec': self._env_spec,
            'skill_info': {'dim_skill': self.dim_skill},
            'use_encoder': self.use_encoder,
            # ... other args as in init
        }
        self.skill_policy._add_base_policy(PolicyEx, policy_args, with_lambda=True)
        # Update optimizer to include new parameters
        self._optimizer._optimizers['skill_policy'] = torch.optim.Adam(self.skill_policy.parameters(), lr=_finalize_lr(None))  # Reinitialize optimizer with all params
    ...
    def _optimize_op(self, metrics, internal_vars):
        ...
        self._gradient_descent(
            metrics['LossSacp'],
            optimizer_keys=['skill_policy'],  # Updates all base policies in cascade
        )
